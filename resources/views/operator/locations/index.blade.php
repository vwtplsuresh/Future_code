<x-operator-layout>
    <x-slot name="header">
        <h2 class="flex justify-between text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
            {{ __('Locations') }}
            @can('create location')
                <x-a-button :href="route('operator.locations.create')" >Create New Location</x-a-button>
            @endcan
        </h2>
    </x-slot>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-right">
                                                <x-dropdown align="right" width="48">
                                                    ...
                                                    <x-slot name="content">
                                                        @can('edit location')
                                                            <x-dropdown-link :href="route('operator.locations.edit', $location->id )">
                                                                {{ __('Edit') }}
                                                            </x-dropdown-link>
                                                        @endcan

                                                        @can('view location')
                                                            <x-dropdown-link :href="route('operator.locations.show', $location->id )">
                                                                {{ __('View') }}
                                                            </x-dropdown-link>
                                                        @endcan
                                                        @can('delete location')
                                                            <!-- Authentication -->
                                                            <form method="POST" id="delete-form" action="{{ route('operator.locations.destroy', $location->id ) }}">
                                                                @csrf
                                                                @method('DELETE')
                                                                <x-dropdown-link :href="route('operator.locations.destroy', $location->id)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                                    {{ __('Delete') }}
                                                                </x-dropdown-link>
                                                            </form>
                                                        @endcan
